...
describe "/api/v1/projects", :type => :api do
	...
	context "projects viewable by this user" do
		...
		it "JSON" do
			...
			# tmpJSONFile = File.new("projectsspec_jsonresponse.txt","w")
			# tmpJSONFile.syswrite(projects)
			# tmpJSONFile.close

			projects.any? do |p|
				# tmpJSONFile = File.new("presponse.txt","w")
				# tmpJSONFile.syswrite(p)
				# tmpJSONFile.close
				p["name"] == "Ticketee"
			end.should be_true
	...
	context "show" do
		...
		it "JSON" do
			...
			logFile = File.new("unsJson.txt","w")
			logFile.syswrite(last_response.body)
			logFile.close
